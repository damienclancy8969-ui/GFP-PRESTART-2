...
import { getCladdingStyle, getCladdingTexture, type CladdingProfile } from "../../utils/claddingTextures";
...
function CladdingMaterial({ profile, color, widthMetres, heightMetres }: { profile: CladdingProfile; color: string; widthMetres: number; heightMetres: number }) {
  const style = getCladdingStyle(profile);
  const bumpMap = useMemo(() => getCladdingTexture(profile, widthMetres, heightMetres), [profile, widthMetres, heightMetres]);
  return (
...
function FootprintWalls({
  ...
  const box = useMemo(() => new THREE.Box3().setFromBufferAttribute(geometry.attributes.position as THREE.BufferAttribute), [geometry]);
  const perimeter = (box.max.x - box.min.x + (box.max.z - box.min.z)) * 2;

  return (
    <mesh geometry={geometry} castShadow receiveShadow>
      <CladdingMaterial profile={profile} color={color} widthMetres={perimeter} heightMetres={wallHeight} />
    </mesh>
  );
}
...
export function HouseModel({ colours, pattern, width, depth, wallHeight, roofPitch, outline }: Props) {
  ...
  return (
    <group>
      {hasOutline ? (
        <FootprintWalls outline={outline!} wallHeight={wallHeight} color={colours.wallPrimary} profile={pattern.primary} />
      ) : (
        <>
          {/* Primary wall block */}
          <mesh position={[0, wallHeight / 2, 0]} castShadow receiveShadow>
            <boxGeometry args={[width, wallHeight, depth]} />
            <CladdingMaterial profile={pattern.primary} color={colours.wallPrimary} widthMetres={width + depth} heightMetres={wallHeight} />
          </mesh>
          ...
          <mesh position={[width / 2 - secondaryWidth / 2 + 0.01, wallHeight / 2, 0]} castShadow receiveShadow>
            ...
            <CladdingMaterial profile={pattern.secondary} color={colours.wallSecondary} widthMetres={secondaryWidth + depth} heightMetres={wallHeight} />
          </mesh>
        </>
      )}
      ...
      <mesh position={[0, wallHeight + 0.02, depth / 2 + OVERHANG]}>
        ...
        <meshStandardMaterial color={colours.fascia} roughness={0.4} metalness={0.12} />
      </mesh>
      <mesh position={[0, wallHeight + 0.02, -depth / 2 - OVERHANG]}>
        <boxGeometry args={[width + OVERHANG * 2, 0.09, 0.05]} />
        <meshStandardMaterial color={colours.fascia} roughness={0.4} metalness={0.12} />
      ...
      <mesh position={[width / 4, wallHeight / 2 + 0.2, depth / 2 + 0.015]}>
        ...
        <meshStandardMaterial color={colours.windowFrame} roughness={0.4} metalness={0.2} />
      </mesh>
